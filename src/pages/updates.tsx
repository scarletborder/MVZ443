...
const UpdatesPage = () => {
    ...
    const [update_logs, setUpdateLogs] = useState<string>('');
    const navigate = useNavigate();

    useEffect(() => {
        fetch(`${publicUrl}/updates/${language}.md`)
            .then((res) => {
                if (res.ok) {
                    return res.text();
                } else {
            ...
            .catch((err) => {
                console.error('fetch update log fails', err);
                setUpdateLogs('Failed to fetch update log: ' + err);
            });
    }, [language]);
