...
export function StartArc(game: Game, x1: number, y1: number, x2: number, y2: number,
  ...
  const arcHeight = (_arcHeight || 200) * PositionManager.Instance.scaleFactor;
  ...
  const tempImage = game.add.image(x1, y1, texture).setDepth(DepthUtils.getProjectileDepth('projectile', 0));
