...
import { Wave, Monster } from "../../models/IRecord";
...
namespace StageHelper {
  ...
  export function generateStageScript(stage: StageScript, random: seedrandom.PRNG, stageID: number): Wave[] {
    ...
    function GetMobWeight(mobId: number, waveId: number): number {
      return MonsterLibrary.GetModel(mobId)?.weight(waveId) ?? 0;
    }
    ...
    function CanMobAppear(mobId: number, waveId: number): boolean {
      ...
      const model = MonsterLibrary.GetModel(mobId);
      if (!model) return false;

      if (model.leastWaveIDByStageID) {
        const leastWaveID = model.leastWaveIDByStageID(stageID);
        if (leastWaveID <= waveId) return true;
      }

      if (model.leastWaveID && model.leastWaveID > waveId) return false;
      return true;
    }
    ...
    function generateMobs(levelSum: number, waveId: number): { [mobId: number]: number } {
      ...
      for (const mobModel of MonsterLibrary.Models.values()) {
        const mobId = mobModel.mid;
        if (mobModel.weight(waveId) === 0) continue;
        mobKeys.push(mobId);
      }
      ...
      while (remainingLevel > 1) {
        ...
        const mobLevel = MonsterLibrary.GetModel(mobId)?.level ?? 0;
    ...
    function getAllTimes(waveId: number): { duration: number; minDelay: number; maxDelay: number } {
      ...
      const maxDelay = (_flagWaves.includes(waveId + 1) ? 40 : 20) + randomInt(0, 6); // 下波为flag
      let minDelay: number;
      if (waveId === 1) {
        minDelay = 14;
      } else if (waveId === 2) {
  ...
  export function generateBossWaveScript(baseLevelSum: number, seedrandom: seedrandom.PRNG, AllowedMobs: number[], callIdx: number = 0): Monster[] {
    ...
    for (const mobId of AllowedMobs) {
      ...
      if (model) {
        mobKeys.push(mobId);
        weights.push(model.weight(WAVE_ID));
      }
    }
