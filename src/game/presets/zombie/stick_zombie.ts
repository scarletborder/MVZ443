...
class StickZombie extends EnhancedZombie {
  ...
  constructor(scene: Game, col: number, row: number, texture: string, waveID: number) {
    ...
    const stick = scene.add.sprite(topX, topY, 'attach/hd_stick');
  ...
  public startAttacking(plant: IPlant): void {
    ...
    if (this.hasStick) {
      this.hasStick = false;
      // 进行翻阅操作
      this.vaultTargetX = this.game.positionCalc.gridOffsetX + (plant.col - 0.25) * this.game.positionCalc.GRID_SIZEX;
      this.isFlying = true;
