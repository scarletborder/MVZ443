...
export default class IObstacle extends Phaser.GameObjects.Sprite {
  scene: Game;
  ...
  constructor(scene: Game, x: number, y: number, waveID: number, texture: string, param: ObstacleParam) {
    ...
    // 创建 RAPIER 物理体（kinematic，不受力影响）
    scene.rapierPhysics.createBody(this, size.sizeX, size.sizeY, true);

    this.setOrigin(0.5, 1);

    const { row } = scene.positionCalc.getGridByPos(x, y);
    this.setDepth(DepthUtils.getPlantBasicDepth(row));
