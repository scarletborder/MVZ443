...
class Warden extends IGolem {
  ...
  doRound(skid: number, callback: () => void) {
    ...
    if (this.game?.plantsManager?.planted.has(key)) {
      const list = this.game?.plantsManager?.planted.get(key);
      if (list) {
  ...
  digToNewPosition(isBack: boolean = false, callback: () => void = () => { }) {
    ...
    // 禁用物理体
    this.game.rapierPhysics.disableBody(this);
    this.digTo(x, y);
    this.game.time.delayedCall(6000, () => {
      if (this && this.game) {
        // 启用物理体
        const size = this.game.positionCalc.getZombieBodySize();
        this.game.rapierPhysics.enableBody(this, size.sizeX, size.sizeY * 0.9, false);
        callback();
      }
    });
  ...
  setHealth(value: number) {
    ...
    PhaserEventBus.emit(EVENT_BossHealth, { health: this.health * 100 / this.maxHealth });
  }
  ...
  destoryZombie(): void {
    ...
    super.destoryZombie();
    PhaserEventBus.emit(EVENT_BossHealth, { health: -1 }); // death
  }
