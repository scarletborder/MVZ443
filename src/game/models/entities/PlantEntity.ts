...
export abstract class PlantEntity extends CombatEntity {
  ...
  public useStarShards() {
    this.playStarShardsBurstEffect();
    this.model.onStarShards(this);
  }

  private playStarShardsBurstEffect() {
    const scale = PositionManager.Instance.scaleFactor;
    const startX = this.x;
    const startY = this.y - PositionManager.Instance.GRID_SIZEY * 0.08;
    const burstCount = 12;
    ...
    for (let i = 0; i < burstCount; i++) {
      ...
      const star = this.scene.add.image(startX, startY, "starshards")
        .setOrigin(0.5)
        .setScale(Phaser.Math.FloatBetween(0.18, 0.28) * scale)
        .setAlpha(0)
        .setAngle(Phaser.Math.Between(0, 360))
        .setDepth(this.baseDepth + 40);
    ...
    const glow = this.scene.add.circle(startX, startY, 8 * scale, 0xfff2a6, 0.62)
      .setDepth(this.baseDepth + 39);
    this.scene.tweens.add({
      targets: glow,
      y: startY - 22 * scale,
      scale: 3.2,
